<template>
  <v-dialog content-class="elevation-0" v-bind="dialogarticulo" persistent max-width="40rem">
    <v-card v-on:keyup.enter="submit()" class="cont-card">
      ...
      <form>
        <v-row>
          <v-col sm="3" md="6">
            <v-text-field :rules="nameRules" v-model="name" type="text" :counter="100" label="Nombre artículo"
              required></v-text-field>
          </v-col>
          <v-col sm="3" md="6">
            <v-text-field :rules="numberRules" v-model="cant" type="number" :counter="10" label="Cantidad artículo"
              required></v-text-field>
          </v-col>
        </v-row>
        <v-row align="center">
        ...
        <v-row>
          ...
          <v-col sm="6" md="6">
            <v-select v-model="selectst" :items="itemstst" item-text="nombre_status" item-value="id" label="Estatus"
              required></v-select>
          </v-col>
        </v-row>
        <v-row>
          <v-col sm="3" md="6">
            <v-select v-model="selectr" :items="itemsr" item-text="nombre_rack" item-value="id" label="Ubicación rack"
              required></v-select>
          </v-col>
          <v-col sm="3" md="6">
        ...
        <v-row>
          <v-col>
            <v-file-input label="Subir fotografìa" v-model="photo" prepend-icon="mdi-camera"></v-file-input></v-col>
        </v-row>
        <v-row><v-col>
            <v-textarea v-model="descripcion_articulo" label="Descrpción" type="text"></v-textarea> </v-col></v-row>
      </form>
      <v-card-actions><v-spacer></v-spacer>
        <v-btn color="grey darken-1" @click="clear" outlined> Limpiar </v-btn>
        <v-btn color="yellow darken-2" class="mr-4" v-shortkey="['enter']" @shortkey="submit" @click="submit" outlined>
          Guardar {{ nameat }}
        </v-btn>
      </v-card-actions>
    </v-card>
...
export default {
  ...
  methods: {
    onClose() {
      ...
      this.$emit("dialogarticulo", false);
