<template>
  <v-dialog content-class="elevation-0" v-bind="parentdialog" max-width="20rem" persistent>
    <v-card v-on:keyup.enter="submit()" class="cont-card" elevation="2">
      ...
      <v-row justify-sm="center">
        <v-col sm="6" md="12" lx="13">
          <v-text-field v-model="name_rol" :counter="10" label="Nombre rol" required></v-text-field>
        </v-col>
      </v-row>
      <v-sapacer></v-sapacer>
      <v-btn @click="clear" outlined> Limpiar </v-btn>
      <v-btn class="mr-4" v-shortkey="['enter']" @shortkey="submit" v-on:click="submit" outlined>
        Guardar
      </v-btn>
    </v-card>
  </v-dialog>
