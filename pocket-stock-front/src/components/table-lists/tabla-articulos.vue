<template>
  <v-app id="inspire">
    <v-row>
      <v-col sm="8" md="5">
        <v-text-field label="Buscar artículo" placeholder="Nombre, cantidad, categoria, tipo ...." class="mx-4"
          v-model="search" id="onsearch" />
      </v-col>
    </v-row>
    <v-data-table :headers="headers" :items="articulosArray" :expanded.sync="expanded" sort-by="cantidad_articulo"
      class="elevation-2" :search="search" show-expand :custom-filter="filterOnlyCapsText.toUpperCase">
      ...
      <template v-slot:top>
        <v-toolbar flat>
          ...
          <v-dialog v-bind="dialog" min-width="300px" max-width="600px">
            <v-card>
              <v-card-title>
              ...
              <v-card-text>
                <v-container>
                  <v-row>
                    ...
                    <v-col sm="4" md="6">
                      <v-select v-model="selectr" :items="itemsr" item-text="nombre_rack" item-value="id"
                        label="Ubicación rack" required></v-select>
                    </v-col>
                    <v-col sm="4" md="6">
                      <v-select v-model="selectT" :items="itemsT" item-text="nombre_travesano" item-value="id"
                        label="Ubicación travesaño" required></v-select>
                    </v-col>
                  </v-row>
                  <v-row><v-col>
          ...
          <v-dialog v-bind="dialogDelete" max-width="500px">
            <v-card>
          ...
          <v-dialog v-bind="dialogDetail" transition="dialog-bottom-transition" max-width="600">
            <template v-slot:default="dialogDetail">
              <v-card :key="count">
                ...
                <v-card-actions class="justify-end">
                  <v-file-input v-model="photo" prepend-icon="mdi-camera" hide-input label="File input"></v-file-input>
                  <v-btn v-show="photo != null" text color="orange" @click="photochange()">Subir Imagen</v-btn>
  ...
  methods: {
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        url = `${url}?${"nombre_articulo=" + send.nombre_articulo}&${"cantidad_articulo=" + send.cantidad_articulo
          }&${"descripcion_articulo=" + send.descripcion_articulo}&${"categoria_id=" + this.selectc
          }&${"tipo_id=" + this.selectt}&${"marca_id=" + this.selectm}&${"proveedor_id=" + this.selectp
          }&${"status_id=" + this.selectst}&${"rack_id=" + this.selectr}&${"travesano_id=" + this.selectT
          }`;
